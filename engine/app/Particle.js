...
export default class Particle {
   ...
   static burst(x, y, w, h, qty) {
      ...
      for (var i = 0; i < num; i++) {
           var c1 = this.rgbFromHex(this.settings.colorEnd)
           var c2 = this.rgbFromHex(this.settings.colorStart)
           var life = CSMath.iRandomRange(this.settings.lifeMin, this.settings.lifeMax)
           var dir = CSMath.iRandomRange(this.settings.dirMin, this.settings.dirMax)
           var speed = CSMath.iRandomRange(this.settings.speedMin, this.settings.speedMax)
      ...
      for (var i = 0; i < this.obj.particle.list.length; i++) {
          ...
          if (particle.life > 0 && particle.alpha > 0 && particle.size > 0) {
             ...
             if (particle.shape == 'square') {
                  ...
                  Draw.fillRect({ x: cx, y: cy, width: particle.size, height: particle.size })
             } else {
